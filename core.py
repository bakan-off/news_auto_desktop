...
# Вход: один общий пароль, без кодов подтверждения (v1.5.0). Защита от
# перебора — пауза после серии неудач: повторные неверные логины могут
# надолго заблокировать общий аккаунт на стороне почтового сервера.
LOGIN_MAX_ATTEMPTS = 5                 # неверных попыток входа до паузы
...
def validate_submission(
    ...
    if not is_valid_email(email):
        return "Укажите корректный Email автора новости."
    if not title or not desc or desc == placeholder:
